...
$showFlag = ''
if ((Get-Variable -Name 'SHOWCRACKED' -Scope Script -ErrorAction SilentlyContinue) -and -not [string]::IsNullOrWhiteSpace([string]$SHOWCRACKED)) {
  $showFlag = [string]$SHOWCRACKED
}
$potOpt = ''
if ((Get-Variable -Name 'POTFILE' -Scope Script -ErrorAction SilentlyContinue) -and -not [string]::IsNullOrWhiteSpace([string]$POTFILE)) {
  $potOpt = "--potfile-path=$POTFILE"
}
...
# do NOT add $LOOPBACK here for the base run
if ($showFlag) { Add-If -arr ([ref]$argsBase) $showFlag }
if ($potOpt) { Add-If -arr ([ref]$argsBase) $potOpt }
$argsBase += "-m$HASHTYPE"
Add-If -arr ([ref]$argsBase) $HASHLIST
Add-If -arr ([ref]$argsBase) $WORDLIST
...
foreach ($rule in $RULELIST) {
  ...
  $args += '-r'
  $args += $rule
  Add-If -arr ([ref]$args) $LOOPBACK   # safe; -r present
